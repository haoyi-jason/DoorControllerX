...
/**
  * @brief  Poll ADC2 for a single conversion on the given channel.
  *         ADC2 has no interrupt enabled, so plain flag polling is safe.
  * @param  ch  ADC channel (ADC_CHANNEL_x)
  * @retval 12-bit ADC result, or 0 on timeout
  */
static uint16_t adc2_poll(adc_channel_select_type ch)
{
    ...
    return timeout ? (uint16_t)adc_ordinary_conversion_data_get(ADC2) : 0u;
}
...
void motor_set_direction(uint8_t motor_id, uint8_t dir)
{
    ...
    /* DIP_0=1 swaps forward/reverse polarity */
    if (dip & 0x01u) {
...
void motor_enable(uint8_t motor_id)
{
    ...
    oc.oc_mode         = TMR_OUTPUT_CONTROL_PWM_A;
...
{
    return adc2_poll(ADC_CHANNEL_8);
}
...
{
    return adc2_poll(ADC_CHANNEL_9);
}
